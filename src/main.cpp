...
const int heaterPin[numZones] = {8};            // Pins connected to relays for heating elements.  This is an array for each output pin (zone).
...
void htrControl() {
  ...
  for (i = 0; i < numZones; i++) {
    timeNow = millis();
    if (isnan(pidOutput[i])) {
      pidOutput[i] = 9999;
    }
    if (pidOutput[i] >= (timeNow - pidStart)) {
